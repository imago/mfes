...
public:
  ...
  string getIdentifier(){
	  stringstream ss;
	  ss << jobname << "_pmaxh-" << pmaxh << "_pminh-"<< pminh << "_N-" << lsmsN << "_pgrading-" << pgrading << "_poptsteps3d-" << poptsteps_3d;
	  ss << "_poptsteps2d-" << poptsteps_2d << "_pfineness-"<< pfineness << "_t-" << taubin << "_hc-" << hc << "_lap-";
	  ss << lap << "_bmaxh-"<< bmaxh << "_bgrading-" << bgrading << "_boptsteps3d-" << boptsteps_3d << "_boptsteps2d-" << boptsteps_2d;
...
int main(int argc, char* argv[]) {
	...
	while( !in.eof() ) {
		...
		ss >> p.bmaxh >> p.bgrading >> p.boptsteps_3d;
		...
		currentResult.gpProtein = proteinVolume.getNrPoints();
		cout << "points on surface: " << currentResult.gpProtein << endl;
		...
		cout << "points on surface: " << currentResult.gpTotal << endl;
		currentResult.volumeTotal = protein.getTotalVolume();
		cout << "current volume: " << currentResult.volumeTotal << endl;
	...
	cout << pList.size() << " job(s) on queue." << endl;
